...
function getUserFromRequest(req) {
  try {
    // Check if req exists
    if (!req || !req.headers) {
      console.log('⚠️  getUserFromRequest: req or req.headers is missing');
      return null;
    }
    
    const authHeader = req.headers.authorization;
    if (!authHeader) {
      console.log('⚠️  getUserFromRequest: No authorization header');
      return null;
    }

    // Verify token - this will throw if invalid
    const decoded = verifyToken(authHeader);
    console.log('✅ getUserFromRequest: Token verified, user:', decoded.id, 'role:', decoded.role);
    return decoded;
  } catch (error) {
    console.log('⚠️  getUserFromRequest: Token verification failed:', error.message);
    return null;
  }
}
